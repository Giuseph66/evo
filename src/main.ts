...
async function bootstrap() {
  ...
  if (!serverConfig.DISABLE_DOCS) {
    try {
      const swaggerDocument = JSON.parse(readFileSync(join(ROOT_DIR, 'public', 'openapi.json'), 'utf8'));

      // Update swagger server URL dynamically
      ...
      logger.info('Swagger Documentation - ON: /docs');
    } catch (error) {
      logger.warn('Swagger Documentation - Failed to load openapi.json');
    }
  }
